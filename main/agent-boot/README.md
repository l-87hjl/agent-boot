...
A read-only contract repository containing templates, schemas, rules, and protocols that govern AI agent behavior across all workspaces.
...
│   ├── AGENT_RULES.md      # 6 core rules all agents must follow
│   └── LOOP_PROTOCOL.md    # 7-step execution cycle
...
Every agent session follows a 7-step cycle:

```
1. READ ENTRY    → Parse AGENT_LINK.md
2. LOAD BOOT     → Fetch this repo, verify version
3. REHYDRATE     → Load STATE.json, TODO.json
4. VALIDATE      → Check against schemas
5. PLAN          → Determine next actions
6. ACT           → Execute the plan
7. PERSIST       → Save state, commit, push
```
...
1. **Entry Point:** Always read `AGENT_LINK.md` first
...
See `contract/AGENT_RULES.md` for details.
...
Validation is mandatory before any agent action. See `contract/AGENT_RULES.md` Rule 3.
...
The agent:
1. Reads `AGENT_LINK.md` from workspace
2. Fetches boot contract from this repo
